...
num_channels_Y=[64, 64, 64, 64, 64, 1] ## number of channels for hybrid model
kernel_size=7 ## kernel size for local models
dropout=0.2 ## dropout during training
rank=256 ## rank of global model
...
def main(args):
    DG = DeepGLO(Ymat,vbsize=vbsize,\
        ...
        use_time=use_time,\
        dti=dti,\
        svd=svd,period=period,forward_cov = forward_cov)

    DG.train_all_models(y_iters=y_iters,init_epochs=init_epochs)

    result_dic = DG.rolling_validation(Ymat = Ymat,tau=14,n=4,bsize=100,cpu=False,alpha=0.3)
    print(result_dic)

    out_path = Path('/efs/users/rajatse/DeepGLOv2','results','result_dictionary_wiki_' + bool2str(normalize) + '.pkl')
    pickle.dump(result_dic,open(out_path,'wb'))


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--normalize', type=str2bool, required=True,help="normalize for training or not")
